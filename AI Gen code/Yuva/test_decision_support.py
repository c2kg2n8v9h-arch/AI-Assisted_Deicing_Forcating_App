...
class DecisionSupportTests(unittest.TestCase):
    ...
    def test_service_estimate_is_advisory_range(self) -> None:
        estimate = DecisionSupportEngine.estimate_service_time(
            self.flights[0], self.weather
        )
        self.assertLess(
            estimate["range_minutes"]["low"], estimate["estimated_minutes"]
        )
        self.assertGreater(
            estimate["range_minutes"]["high"], estimate["estimated_minutes"]
        )
    ...
    def test_resource_recommendations_require_dispatcher_approval(self) -> None:
        ...
        self.assertTrue(
            all(item["requires_dispatcher_approval"] for item in recommendations)
        )
    ...
    def test_anomalies_are_explainable_and_require_review(self) -> None:
        ...
        unknown = Flight(
            "YU300", "UNKNOWN", self.now - timedelta(minutes=10), "A3", 10
        )
